...
Отдельный локальный сервис для просмотра календарей сотрудников.

Сервис читает только собственный `yandex_calendar_viewer/.env` или переменные окружения контейнера. Связи с соседними проектами и их env-файлами нет.
